...
public class VisaStatusControllerTest {
    ...
    void should_return_status_by_ticketId() {
        //given
        when(visaService.getStatus("T-122")).thenReturn(VisaStatusResponse.builder().status("processing").build());
    ...
    void should_return_failed_status_by_ticketId() {
        //given
        when(visaService.getStatus("T-122")).thenReturn(VisaStatusResponse.builder().status("failed").build());
    ...
    void should_return_unknown_status_when_visa_service_return_null() {
        ...
        //then
        assertTrue(response.getStatusCode().is4xxClientError(), "should return 404 -> actual:" + response.getStatusCode());
        assertNotNull(response.getBody(), "expect response with unknown status, but found null body");
        assertEquals("unknown", response.getBody().getStatus());
    }
    ...
    void should_return_unknown_status_when_visa_service_throw() {
        ...
        //then
        assertTrue(response.getStatusCode().is5xxServerError(), "should return 500 -> actual:" + response.getStatusCode());
        assertNotNull(response.getBody(), "expect response with unknown status, but found null body");
        assertEquals("unknown", response.getBody().getStatus());
    }
}
